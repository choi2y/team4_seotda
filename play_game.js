...
function betting(playerIndex, totalPoint, bettingPoint, isAllIn, foldedPlayers) {
    let player = playerGameInfo[playerIndex];
    let choice = bettingType(player.bettingType);
...
function gameStart() {
    // 수정 /////////////////////
    // 재경기 시 살아남은 플레이어 끼리만, 총 배팅금은 리셋X, 배팅금은 리셋 후 게임 진행

    // 초기 배팅값 초기화, 카드 초기화, pair 초기화
    // 탈락한 플레이어 배팅 타입 -> 다이
    // while(isReplay===1){
    ...
    //         registerBettingButtons();
    //     }
    //     findPair(playerGameInfo);
    //     rankingCards(playerGameInfo);
    //     gameResult(); // 재경기 아니면 isReplay = 0
    // }
